locals {
  input_csv  = file("${path.module}/${var.engine_postgres_input_file}")
  ...
  connection_roles = {
    for connection in keys(var.engine_postgres_connections) : connection => [
  ...
  active_connections = {
    for connection, config in var.engine_postgres_connections : connection => config
